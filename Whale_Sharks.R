...
#filter id with low number of locs and onen shark that the tag popped off
sharks_ssm_input <- sharks_ssm_input |> 
  dplyr::filter(!id %in% c("178950", "280633"))
...
saveRDS(crw_locs_meta_f, "WhaleSharkTracks_SPOT_SPLASH_20250212.rds")
...
frames <- moveVis::frames_spatial(m,
  ...
  #            x = "Longitude", 
  #            y = "Latitude") |>  
  # add_gg(gg = expr(coord_cartesian(xlim = c(143.5, 150.5), ylim = c(-25, -10)))) |>
...
# animate_frames(frames2, out_file = "Whale_Sharks_2024_Animation_20250128_IG-Reel_I.Miller.mov", end_pause = 0, overwrite=TRUE, res=200, fps = 5,
#                width = 1400,
#                height = 2000)
